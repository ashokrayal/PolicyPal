"""
Logging configuration for PolicyPal project.
...
_policy_pal_logger = None


def get_logger(name: str = "PolicyPal") -> logger:
    """
...
def log_function_call(func_name: str, args: dict = None, kwargs: dict = None):
    ...
    logger.debug(f"Calling {func_name}({', '.join(params)})")


def log_function_result(func_name: str, result=None, error=None):
...
def log_data_processing(operation: str, input_size: int, output_size: int = None):
    """Log data processing operations."""
    logger = get_logger()
    if output_size:
        logger.info(f"Data processing: {operation} - Input: {input_size}, Output: {output_size}")
    else:
        logger.info(f"Data processing: {operation} - Input: {input_size}")
